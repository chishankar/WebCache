...
function deleteWord(word, filename) {
  //finds index of word we need to delete
  let wordIndMain = mainIndex.findIndex(wordInd => {return wordInd.w === word});
  //word not found = return
  ...
  //change start values of all other
  let j = wordIndMain + 1;
  ...
  return new Promise(resolve => {
    fs.readFile(filePath, {encoding: 'utf-8'}, function(err,data) {
      ...
      throw err;
        ...
        let dataClean = data.replace(/[~!@#$%^&*()_|\=?;:'",.<>\{\}\[\]\\\/]/gi, '');
        //retrieves all words we have to delete
        let deleteWords = dataClean.toLowerCase().trim().split(/\s+/).filter(function(value,index,self) {return !stopWords.includes(value) && self.indexOf(value) === index;});
        deleteWords.sort();
        ...
        //   //strips word of any special characters
        //   word = word.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '');
        //   deleteWord(word, filename);
        // });
        resolve();
      }
    });
...
if(INDEX_DIRECTORY) {
  var proc = exec('ls test_docs', (err, stdout, stderr) => {
    ...
    addFilesToMainIndex(dirFiles, mainIndex).then(result => {
      ...
      deleteFile("abecant.txt").then(result => {
