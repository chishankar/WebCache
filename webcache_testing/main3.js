    ...
    fs.readFile(filePath, {encoding: 'utf-8'}, function(err,data){
        if (!err) {
          ...
          fileWords.forEach( word => {
            let locations = new Uint32Array(getIndicesOf(word,cleanText));
            fileIndex.push({w: word, a: [fileNum, locations]});
          });
          //returns the list
          resolve(fileIndex);
        } else {
...
function addToMainIndex(fileIndex, mainIndex, fId) {
  ...
  fileIndex.forEach(fileWord => {
    ...
    if (wordInd >= 0) {
      mainIndex[wordInd].a.push(fileWord.a)
    } else {
      mainIndex.push({w: fileWord.w, a: [fileWord.a]});
    }
  });
}
...
function user_search(str) {
  ...
  results.forEach(obj => {
    ...
    console.log(obj.fileName + " at " + locations);
  });
